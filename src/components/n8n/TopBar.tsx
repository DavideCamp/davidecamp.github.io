    ...
    <motion.header style={{ y }} className="sticky top-0 z-50 border-b border-white/60 bg-white/35 backdrop-blur-2xl dark:border-white/10 dark:bg-slate-900/45">
      <div className="relative mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">D-Automazioni</span>
        </div>
        ...
        <nav className="hidden items-center gap-7 md:flex">
          {menu.map((item) => (
            <a
              key={item.label}
              href={item.href}
              data-progress-id={item.href.replace('#', '')}
              className="text-sm text-slate-600/95 transition hover:text-slate-900 dark:text-slate-300 dark:hover:text-white"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <a href="#contatti" className="primary-btn text-center text-xs md:text-sm">
            Prenota una call
          </a>
        </div>
